  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px' }}>
      ...
      <p style={{ color: '#6b7280', marginBottom: 24, fontSize: 14 }}>
        Click the <strong>RTK</strong> button in the bottom-right to open the devtools.
        Try triggering queries and mutations below to see them in the devtools.
      </p>
    ...
    <div style={{ marginBottom: 24, padding: 16, border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff' }}>
      ...
      {users && (
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {users.map((user) => (
            <div key={user.id} style={{ padding: '8px 12px', background: '#f3f4f6', borderRadius: 6, fontSize: 13 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <button onClick={handleAddPost} style={{ ...btnStyle, background: '#7e22ce', color: '#fff' }}>
          + Add Post
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        ...
        <div style={{ flex: 1 }}>
          {posts?.map((post) => (
            <div
              key={post.id}
              onClick={() => onSelectPost(post.id === selectedPostId ? null : post.id)}
              style={{
                padding: '12px 16px',
                marginBottom: 8,
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                <button
                  onClick={(e) => { e.stopPropagation(); updatePost({ id: post.id, title: post.title + ' (updated)' }) }}
                  style={{ ...smallBtnStyle }}
                >
                  Update
                </button>
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); onToggleComments(showComments === post.id ? null : post.id) }}
                  style={{ ...smallBtnStyle }}
                >
    ...
    <div style={{ padding: 16, border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff' }}>
      ...
      {post && (
        <>
          ...
          <p style={{ fontSize: 11, color: '#9ca3af', marginTop: 8 }}>User ID: {post.userId}</p>
        </>
      )}
    </div>
  )
...
function CommentsSection({ postId }: { postId: number }) {
  ...
  return (
    <div style={{ marginTop: 16, padding: 16, border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff' }}>
      <h3 style={{ fontSize: 16, marginBottom: 12 }}>Comments for Post #{postId}</h3>
      {isLoading && <p>Loading comments...</p>}
      {error && <p style={{ color: 'red' }}>Error loading comments</p>}
      {comments?.map((comment) => (
        <div key={comment.id} style={{ padding: 8, marginBottom: 8, background: '#f9fafb', borderRadius: 6, fontSize: 12 }}>
          <strong>{comment.name}</strong>
          <div style={{ color: '#6b7280', marginTop: 4 }}>{comment.body.slice(0, 100)}...</div>
        </div>
      ))}
    </div>
  )
