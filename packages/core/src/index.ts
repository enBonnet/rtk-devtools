...
export function createRTKDevtools(config: RTKDevtoolsConfig): RTKDevtoolsInstance {
  ...
  function refetchQuery(endpointName: string, args: unknown): void {
    ...
    if (endpoint?.initiate) {
      store.dispatch(
        endpoint.initiate(args, { forceRefetch: true }) as never,
      )
    }
  }
 ...
 * ```
 */
export function createDevtoolsMiddleware(config: DevtoolsMiddlewareConfig): Middleware & {
...
function extractEndpointTagTypes(
  ...
  if (Array.isArray(tags)) {
    return tags.map((t) => (typeof t === 'string' ? t : (t as { type: string }).type))
  }
