...
export function createDevtoolsMiddleware(
  ...
  return () => (next) => (action: unknown) => {
    ...
    // Subscriptions
      ...
      type.startsWith(subscriptionsPrefix) ||
      type.includes("/subscriptions/")
    ) {
      handleSubscriptionAction(type, action, now, recorder);
    } else if (
      type === unsubscribePrefix ||
      type.includes("unsubscribeQueryResult")
    ) {
      const meta = extractMeta(action);
...
function extractMeta(action: Record<string, unknown>): ActionMeta {
  ...
  return {
    endpointName:
      (arg?.endpointName as string) ?? (meta?.endpointName as string) ?? "",
    queryCacheKey:
...
function extractInvalidatedTags(
...
function isAction(
  action: unknown,
): action is Record<string, unknown> & { type: string } {
  return (
