...
export function createCacheTimerTracker(defaultKeepUnusedDataFor: number = 60) {
  const timers = new Map<string, { startedAt: number; keepFor: number; endpointName: string }>()
    ...
    for (const [cacheKey, count] of subscriberCounts) {
      if (count === 0) {
        ...
        if (!timers.has(cacheKey)) {
          ...
          const keepFor = endpointKeepFor?.get(endpointName) ?? defaultKeepUnusedDataFor
