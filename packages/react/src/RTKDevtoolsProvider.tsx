...
import type { RTKDevtoolsInstance, DevtoolsSnapshot, RTKQueryApi } from '@rtk-devtools/core'
...
export function useDevtoolsContext(): RTKDevtoolsContextValue {
  ...
  if (!ctx) {
    throw new Error('useDevtoolsContext must be used within RTKDevtoolsProvider')
  }
  return ctx
}
...
export function RTKDevtoolsProvider({
  ...
  const devtoolsRef = useRef<RTKDevtoolsInstance | null>(null)

  if (!devtoolsRef.current) {
    devtoolsRef.current = createRTKDevtools({
      api,
      store,
      maxTimelineEvents,
      throttleMs,
