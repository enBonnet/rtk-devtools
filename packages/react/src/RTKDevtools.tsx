...
import { RTKDevtoolsProvider, useDevtoolsSnapshot, useDevtools } from './RTKDevtoolsProvider'
...
export function RTKDevtools({
  ...
  // Production bailout
  const isDisabled = disabled ?? (typeof process !== 'undefined' && process.env?.NODE_ENV === 'production')
  if (isDisabled) return null

  // Try to get store from react-redux context if not passed
  const store = storeProp ?? useReduxStore()
  if (!store) {
...
function DevtoolsUI({
  ...
  const toggleOpen = useCallback(() => {
    setIsOpen((prev) => {
      const next = !prev
      try { localStorage.setItem(STORAGE_KEY_OPEN, String(next)) } catch { /* noop */ }
      return next
    })
  }, [])
  ...
  const tabs: TabItem[] = [
    ...
    { id: 'mutations', label: 'Mutations', count: snapshot.stats.totalMutations },
    { id: 'tags', label: 'Tags', count: snapshot.tagGraph.length },
    { id: 'subscriptions', label: 'Subs', count: snapshot.stats.totalSubscriptions },
  ...
  return (
    <>
      ...
      />
      <FloatingPanel isOpen={isOpen} position={position} initialSize={panelSize}>
