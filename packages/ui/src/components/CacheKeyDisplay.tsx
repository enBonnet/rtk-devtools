...
export function CacheKeyDisplay({
  cacheKey,
  maxLength = 50,
}: CacheKeyDisplayProps) {
  const { theme } = useTheme();
  const truncated =
    maxLength > 0 && cacheKey.length > maxLength
  ...
  const endpointName =
    parenIndex > -1 ? cacheKey.slice(0, parenIndex) : cacheKey;
  const args = parenIndex > -1 ? cacheKey.slice(parenIndex) : "";
  const truncatedArgs =
    maxLength > 0 && args.length > maxLength - endpointName.length
