...
const eventConfig: Record<TimelineEventType, { label: string; icon: string; getColor: (t: Theme) => string }> = {
  'query-started': { label: 'Query Started', icon: '\u25B6', getColor: (t) => t.status.pending },
  'query-fulfilled': { label: 'Query Fulfilled', icon: '\u2713', getColor: (t) => t.status.fulfilled },
  'query-rejected': { label: 'Query Rejected', icon: '\u2717', getColor: (t) => t.status.rejected },
  'mutation-started': { label: 'Mutation Started', icon: '\u25B6', getColor: (t) => t.status.pending },
  'mutation-fulfilled': { label: 'Mutation Fulfilled', icon: '\u2713', getColor: (t) => t.status.fulfilled },
  'mutation-rejected': { label: 'Mutation Rejected', icon: '\u2717', getColor: (t) => t.status.rejected },
  'invalidation': { label: 'Invalidation', icon: '\u21BB', getColor: (t) => t.brand[500] },
  'cache-entry-removed': { label: 'Cache Removed', icon: '\u2715', getColor: (t) => t.text.tertiary },
  'subscription-added': { label: 'Subscribed', icon: '+', getColor: (t) => t.status.fulfilled },
  'subscription-removed': { label: 'Unsubscribed', icon: '\u2212', getColor: (t) => t.status.rejected },
}
...
export function TimelinePanel({ events, onClear }: TimelinePanelProps) {
  ...
  return (
    <div className={css({ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' })}>
      <div className={css({ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', borderBottom: `1px solid ${theme.border.primary}` })}>
        <input
          type="text"
          value={search}
...
function TimelineEventRow({ event }: { event: TimelineEvent }) {
  ...
  return (
    ...
    >
      {/* Timeline dot */}
      <div className={css({ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '2px' })}>
        <span
          className={css({
            display: 'flex',
            alignItems: 'center',
      ...
      <div className={css({ flex: 1, minWidth: 0 })}>
        ...
        {event.cacheKey && (
          <div className={css({ fontSize: '11px', fontFamily: '"SF Mono", Menlo, monospace', color: theme.text.secondary, marginTop: '1px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' })}>
            {event.cacheKey}
          </div>
        )}
        {event.tags && event.tags.length > 0 && (
          <div className={css({ display: 'flex', flexWrap: 'wrap', gap: '3px', marginTop: '3px' })}>
