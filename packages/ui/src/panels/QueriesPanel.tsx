...
export function QueriesPanel({ queries, onRefetch, onTagClick }: QueriesPanelProps) {
  ...
  const filteredQueries = useMemo(() => {
    ...
    entries.sort((a, b) => {
      if (sortBy === 'name') return a.endpointName.localeCompare(b.endpointName)
    ...
    <div className={css({ display: 'flex', height: '100%', overflow: 'hidden' })}>
      ...
      <div
        className={css({
          ...
          borderRight: selectedEntry ? `1px solid ${theme.border.primary}` : 'none',
          overflow: 'hidden',
        })}
      >
        <SearchFilter
        ...
        <div className={css({ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 12px', borderBottom: `1px solid ${theme.border.secondary}` })}>
          <span className={css({ fontSize: '11px', color: theme.text.tertiary })}>Sort:</span>
          ...
          <span className={css({ marginLeft: 'auto', fontSize: '11px', color: theme.text.tertiary })}>
          ...
          {filteredQueries.length === 0 ? (
            ...
            filteredQueries.map((entry) => (
              <QueryListItem
                ...
                onSelect={() => setSelectedKey(entry.cacheKey === selectedKey ? null : entry.cacheKey)}
              />
            ))
          )}
        </div>
      ...
      {/* Detail */}
      {selectedEntry && (
        <div className={css({ flex: '0 0 50%', overflow: 'auto', padding: '12px' })}>
          <QueryDetail
            entry={selectedEntry}
            onRefetch={onRefetch}
  ...
  return (
    ...
    >
      ...
      {entry.subscriberCount > 0 && (
        <span className={css({ fontSize: '10px', color: theme.text.tertiary, whiteSpace: 'nowrap' })}>
    ...
    <div className={css({ display: 'flex', flexDirection: 'column', gap: '12px' })}>
      ...
      <div className={css({ display: 'flex', alignItems: 'center', justifyContent: 'space-between' })}>
        <div className={css({ display: 'flex', alignItems: 'center', gap: '8px' })}>
          ...
          <span className={css({ fontWeight: 600, fontSize: '14px' })}>{entry.endpointName}</span>
        </div>
        {onRefetch && (
          <button
      ...
      <Section title="Cache Key">
        <code className={css({ fontSize: '11px', color: theme.text.secondary, wordBreak: 'break-all' })}>
          {entry.cacheKey}
        </code>
      </Section>
      ...
      <Section title="Timing">
        <div className={css({ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '4px 12px', fontSize: '12px' })}>
          {entry.startedTimeStamp && (
            <>
              <span className={css({ color: theme.text.tertiary })}>Started</span>
              <TimeAgo timestamp={entry.startedTimeStamp} />
            </>
          )}
          {entry.fulfilledTimeStamp && (
            <>
              <span className={css({ color: theme.text.tertiary })}>Fulfilled</span>
              <TimeAgo timestamp={entry.fulfilledTimeStamp} />
            </>
          )}
          {entry.duration !== undefined && (
            <>
              <span className={css({ color: theme.text.tertiary })}>Duration</span>
              <TimingBar duration={entry.duration} />
            </>
          )}
        </div>
...
function SubscriberInfo({ entry }: { entry: DevtoolsQueryEntry }) {
  ...
  const polling = entry.pollingInterval ? ` (polling every ${entry.pollingInterval / 1000}s)` : ''
  return (
    <span className={css({ fontSize: '12px', color: theme.text.secondary })}>
      {entry.subscriberCount} active subscriber{entry.subscriberCount !== 1 ? 's' : ''}
      {polling}
    </span>
  )
}
...
function Section({
  ...
  return (
    <div>
      ...
      >
        <span className={css({ fontSize: '9px' })}>{collapsed ? '\u25B6' : '\u25BC'}</span>
        {title}
      </div>
      {!collapsed && (
