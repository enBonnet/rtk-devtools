...
export function SubscriptionsPanel({ queries }: SubscriptionsPanelProps) {
  ...
  const entries = useMemo(() => {
    ...
    if (showExpiring) {
      items = items.filter(
        (e) => e.subscriberCount === 0 && e.cacheLifetimeRemaining !== null,
      );
    }
  ...
  const totalSubs = useMemo(
    () =>
      Array.from(queries.values()).reduce(
        (sum, q) => sum + q.subscriberCount,
        0,
      ),
    [queries],
  );
  const expiringCount = useMemo(
    () =>
      Array.from(queries.values()).filter(
        (q) => q.subscriberCount === 0 && q.cacheLifetimeRemaining !== null,
      ).length,
    [queries],
  );
        ...
        {expiringCount > 0 && (
          <button
            ...
            className={css({
              ...
              fontSize: "11px",
              cursor: "pointer",
              backgroundColor: showExpiring
                ? `${theme.status.rejected}20`
                : "transparent",
...
function SubscriptionRow({ entry }: { entry: DevtoolsQueryEntry }) {
  const { theme } = useTheme();
  const isExpiring =
    entry.subscriberCount === 0 && entry.cacheLifetimeRemaining !== null;
    ...
    >
      ...
      <td className={css({ padding: "6px 12px", textAlign: "right" })}>
        {isExpiring ? (
          ...
          <span
            className={css({ color: theme.status.fulfilled, fontSize: "11px" })}
          >
            active
          </span>
        )}
